        ...
        for tk in self.mtl_list:
            ...
            for sub_tk in self.sub_mtl_list[tk]:
                ...
                self.model_lstm_dict[tk][sub_tk] = (
                    nn.LSTM(nin, nhi, nlayers, batch_first=True)
                    .double()
                    .to(self.device)
                )
            ...
            for tk in self.mtl_list:
                for sub_tk in self.sub_mtl_list[tk]:
                    ...
                    start_ids = np.random.permutation(
                        list(
                            range(
                                self.Xtrain_tasks[tk][sub_tk].size(0) - self.seq_len - 1
                            )
                        )
                    )[: self.batch_size]
                    XYbatch = torch.stack(
                        [
                            self.Xtrain_tasks[tk][sub_tk][i : i + self.seq_len + 1]
                            for i in start_ids
                        ],
                        dim=0,
                    )
                    ...
                    (hidden, cell) = self.get_hidden(
                        self.batch_size, self.nlayers, self.nhi
                    )
